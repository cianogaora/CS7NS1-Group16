...
# The location contains two kinds of data (longitude and latitude)
class Location:
    # Latitude and longitude data generation interval
    def __init__(self):
        ...
        self.final_loc = {}
    # Randomly generate decimals and customize the number of decimal places
    def generate_decimal(self, min, max, numbers):
        ...
        return self.decimal
    # Define the data change interval and ensure that the data does not exceed this interval after the change
    def change_longitude(self):

        latitude_change = self.generate_decimal(-1, 1, 4)
        longitude_change = self.generate_decimal(0, 2, 4)

        while True:
            if 180 >= self.longitude + longitude_change >= -180:
                self.longitude = self.longitude + longitude_change
            elif self.longitude + longitude_change > 180:
                self.longitude -= 1
            elif self.longitude + longitude_change < -180:
                self.longitude += 1
            if 90 >= self.latitude + latitude_change >= -90:
                self.latitude = self.latitude + latitude_change
            elif self.latitude + latitude_change > 90:
                self.latitude -= 1
            elif self.latitude + latitude_change < -90:
                self.latitude += 1
    ...
    def generate_location(self):
        ...
        t1.start()
    # Return the data in the form of a dictionary with timestamp as the key value
